...
/* ---- EXPORT MODAL & EXCEL GENERATION ---- */
openExportModalBtn.addEventListener("click", () => exportModal.classList.add("show"))
...
confirmExportBtn.addEventListener("click", () => {
    ...
    if (selectedScope === "expense") {
        items = items.filter(t => t.type === "expense")
    } else if (selectedScope === "income") {
        items = items.filter(t => t.type === "income")
    }

    if (!items.length) {
        alert("No transactions found for the selected scope.")
        return
    }
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Finia Report")

    // Export file
    const dateStr = new Date().toISOString().split("T")[0]
    XLSX.writeFile(workbook, `finia-${selectedScope}-report-${dateStr}.xlsx`)
